...
const transaccionController = {
  ...
  /**
   * @async
   * @function getTransaccionIdByPujaId
   * @description Devuelve solo el ID de la transacción asociada a una puja (para administradores).
   * @param {object} req - Contiene `req.params.pujaId`.
   * @param {object} res - Objeto de respuesta de Express.
   */
  async getTransaccionIdByPujaId(req, res) {
    try {
      const { pujaId } = req.params;
      const transaccionId =
        await transaccionService.findTransaccionIdByPujaId(pujaId);
      if (!transaccionId) {
        return res.status(404).json({
          error: `No se encontró ninguna transacción asociada a la puja con ID ${pujaId}`,
        });
      }
      res.status(200).json({ transaccion_id: transaccionId });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  },
